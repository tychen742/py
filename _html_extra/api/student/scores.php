...
$summary = py_list_student_score_summary($pdo, (int) $student['student_user_id']);
$attempts = py_list_student_attempts($pdo, (int) $student['student_user_id'], 100);
?>
<!doctype html>
<html lang="en">
<head>
      ...
      <table>
        ...
        <?php foreach ($rows as $row): ?>
          <tr>
            ...
            <td><?php echo py_h($isSummary ? $row['best_score'] : $row['score']); ?> / <?php echo py_h($row['max_score']); ?></td>
            <?php if ($isSummary): ?>
              <td><?php echo py_h($row['attempt_count']); ?></td>
              <td><?php echo py_h($row['last_submitted_at']); ?></td>
            <?php else: ?>
              <td><span class="status"><?php echo py_h($row['canvas_sync_status']); ?></span></td>
              <td><?php echo py_h($row['submitted_at']); ?></td>
            <?php endif; ?>
          </tr>
        <?php endforeach; ?>
        </tbody>
