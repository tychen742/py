...
<body>
  ...
  <main class="shell">
    ...
    <div class="table-wrap">
      <table>
        ...
        <?php foreach ($users as $user): ?>
          ...
          <tr class="user-row" tabindex="0" onclick="document.getElementById('user-dialog-<?php echo py_h($user['id']); ?>').showModal()" onkeydown="if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); this.click(); }">
            ...
            <td><?php echo $user['last_login_at'] ? py_h($user['last_login_at']) : ''; ?></td>
          </tr>
        <?php endforeach; ?>
        </tbody>
      </table>
    ...
    <?php foreach ($users as $user): ?>
      <?php $isSelf = (int) $user['id'] === (int) $admin['id']; ?>
      <dialog class="user-dialog" id="user-dialog-<?php echo py_h($user['id']); ?>">
        <form method="post" class="modal-form">
          <input type="hidden" name="action" value="update_user">
          <input type="hidden" name="id" value="<?php echo py_h($user['id']); ?>">
          <div>
            <h2>Edit User</h2>
            <p><?php echo py_h($user['email']); ?></p>
          </div>
