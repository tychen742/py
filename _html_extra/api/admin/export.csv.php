...
header('Content-Disposition: attachment; filename="py_assignment_scores.csv"');
...
foreach (py_list_attempts($pdo, 10000) as $attempt) {
    if (py_is_report_excluded_student($pdo, $attempt['student_identifier'] ?? null, $attempt['canvas_user_id'] ?? null)) {
        continue;
    }
