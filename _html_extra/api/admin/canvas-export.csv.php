...
fputcsv($out, ['Points Possible', '', '', '', '', py_canvas_score_value($quiz['max_score'] ?? 10)]);
...
foreach (py_best_attempts_by_identifier($pdo, $quizId) as $attempt) {
    ...
    fputcsv($out, [
        ...
        $studentIdentifier,
        '',
        py_canvas_score_value($attempt['score'] ?? ''),
    ]);
}
...
function py_is_canvas_sis_login_id(string $value): bool
{
