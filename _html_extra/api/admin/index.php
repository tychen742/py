...
try {
    ...
    echo '<!doctype html><meta charset="utf-8"><title>Python Quiz Admin</title>';
    echo '<h1>Python Quiz Admin</h1>';
    echo '<p>Database is not configured. Configure MySQL/MariaDB in <code>/var/www/py_private/quiz_config.php</code>.</p>';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'login') {
    if (py_login_admin($pdo, (string) ($_POST['email'] ?? ''), (string) ($_POST['password'] ?? ''))) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'sync') {
    ...
    $summary = py_sync_pending_attempts($pdo, $config, 100);
    $notice = sprintf(
...
$attempts = py_list_attempts($pdo, 300);
...
<head>
  ...
  <title>Python Quiz Admin</title>
  <style><?php echo admin_css(); ?></style>
</head>
<body>
  <main class="shell narrow">
    <h1>Python Quiz Admin</h1>
    <?php if ($error !== null): ?><p class="alert error"><?php echo py_h($error); ?></p><?php endif; ?>
    ...
    <div class="table-wrap">
      <table>
        ...
        <?php foreach ($attempts as $attempt): ?>
          ...
          <tr>
            <td><?php echo py_h($attempt['submitted_at']); ?></td>
            <td><?php echo py_h($attempt['student_identifier'] ?: $attempt['canvas_user_id'] ?: 'Unknown'); ?></td>
            <td><?php echo py_h($attempt['quiz_id']); ?></td>
            <td><?php echo py_h($attempt['score']); ?> / <?php echo py_h($attempt['max_score']); ?></td>
            <td>
              <span class="status"><?php echo py_h($attempt['canvas_sync_status']); ?></span>
              <?php if (!empty($attempt['canvas_sync_error'])): ?>
                <small><?php echo py_h($attempt['canvas_sync_error']); ?></small>
              <?php endif; ?>
            </td>
            <td class="answers"><?php echo py_h(format_answers(is_array($answers) ? $answers : [])); ?></td>
          </tr>
        <?php endforeach; ?>
        </tbody>
      </table>
