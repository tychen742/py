...
set_exception_handler(static function (Throwable $exception): void {
    error_log('Python lab API error: ' . $exception->getMessage());
...
if (py_canvas_ready($config, $identity)) {
    try {
        if (py_is_score_at_least_best($pdo, $labId, $identity, (float) $graded['score'])) {
        ...
        error_log('Python lab best-score check failed before Canvas sync: ' . $exception->getMessage());
        $sync = py_sync_canvas_grade($config['canvas'], $identity, (string) $graded['score']);
    }
}
...
try {
    $attemptSummary = py_attempt_summary($pdo, $labId, $identity);
} catch (Throwable $exception) {
    error_log('Python lab attempt summary failed: ' . $exception->getMessage());
}
