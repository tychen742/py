...
set_exception_handler(static function (Throwable $exception): void {
    error_log('Python homework API error: ' . $exception->getMessage());
...
$homework = py_homework_definition($homeworkId);
if ($homework === null) {
...
$graded = py_grade_homework_attempt($homework, $answers, $code, $config['lab_grader'] ?? []);
...
if (!py_student_identifier_allowed($config, $identity['student_identifier'])) {
    respond(403, [
        'ok' => false,
        'error' => 'student_not_allowed',
        'message' => 'This submission is not enabled for this course roster.',
    ]);
}
...
if (py_canvas_ready($config, $identity)) {
    try {
        if (py_is_score_at_least_best($pdo, $homeworkId, $identity, (float) $graded['score'])) {
        ...
        error_log('Python homework best-score check failed before Canvas sync: ' . $exception->getMessage());
        $sync = py_sync_canvas_grade($config['canvas'], $identity, (string) $graded['score']);
    }
}
...
try {
    $attemptSummary = py_attempt_summary($pdo, $homeworkId, $identity);
} catch (Throwable $exception) {
    error_log('Python homework attempt summary failed: ' . $exception->getMessage());
}
