...
function py_quiz_definition(string $quizId): ?array
{
    $quizzes = [
        ...
        'ch02-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch02',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'C',
        ...
        'ch03-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch03',
            'questions' => [
                'q1' => 'B',
                'q2' => 'A',
                'q3' => 'C',
        ...
        'ch04-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch04',
            'questions' => [
                'q1' => 'A',
                'q2' => 'C',
                'q3' => 'B',
        ...
        'ch05-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch05',
            'questions' => [
                'q1' => 'B',
                'q2' => 'A',
                'q3' => 'B',
        ...
        'ch06-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch06',
            'questions' => [
                'q1' => 'A',
                'q2' => 'C',
                'q3' => 'B',
        ...
        'ch07-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch07',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'B',
        ...
        'ch08-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch08',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'A',
        ...
        'ch09-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch09',
            'questions' => [
                'q1' => 'B',
                'q2' => 'A',
                'q3' => 'B',
        ...
        'ch10-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch10',
            'questions' => [
                'q1' => 'B',
                'q2' => 'A',
                'q3' => 'B',
        ...
        'ch11-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch11',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'B',
        ...
        'ch12-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch12',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'A',
        ...
        'ch13-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch13',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'A',
        ...
        'ch14-preview' => [
            ...
            'assignment_slug' => 'preview',
            'max_score' => 10,
            'canvas_assignment_column' => 'preview_ch14',
            'questions' => [
                'q1' => 'A',
                'q2' => 'B',
                'q3' => 'C',
...
function py_list_admin_report_assignments(PDO $pdo): array
{
    $assignments = [];
    foreach (py_all_assignment_definitions() as $assignmentId => $definition) {
    ...
    foreach ($stmt->fetchAll() as $row) {
        $assignmentId = py_canonical_assignment_id((string) ($row['quiz_id'] ?? ''));
...
function py_list_admin_report_assignment_numbers(PDO $pdo): array
{
    $numbers = [];
    foreach (array_keys(py_all_assignment_definitions()) as $assignmentId) {
...
function py_list_admin_report_students(PDO $pdo): array
{
    ...
    foreach ($adminRows as $adminRow) {
        foreach (['email', 'student_identifier', 'canvas_user_id'] as $field) {
            $identifier = py_normalize_student_identifier((string) ($adminRow[$field] ?? ''));
            if ($identifier !== '') {
    ...
    $users = $pdo->query(
        'SELECT DISTINCT u.display_name, u.role, u.student_identifier, a.student_identifier AS attempt_identifier
         FROM py_quiz_attempts a
         LEFT JOIN py_quiz_users u ON u.id = a.student_user_id
    ...
    foreach ($users as $user) {
        ...
        if ($identifier === '') {
            $identifier = py_normalize_student_identifier((string) ($user['attempt_identifier'] ?? ''));
        }
        if ($identifier === '') {
            continue;
        }
        ...
        $label = $displayName !== '' && py_normalize_student_identifier($displayName) !== $identifier
...
function py_list_admin_score_report(
    PDO $pdo,
...
): array
{
    ...
    $studentFilter = $studentFilter !== null ? py_normalize_student_identifier($studentFilter) : null;
...
function py_upsert_lti_user(PDO $pdo, array $claims): ?int
{
    ...
    $stmt = $pdo->prepare('SELECT id, role FROM py_quiz_users WHERE ' . $where . ' LIMIT 1');
    $stmt->execute($email !== ''
...
function py_admin_nav(string $active): string
{
    ...
    foreach ($items as $key => [$href, $label]) {
        ...
        if ($key === 'logout') {
            $links .= '<a class="' . py_h($class . ' icon-link') . '" href="' . py_h($href) . '" aria-label="Log out" title="Log out">'
