...
- `1300-intro-apis.ipynb`: Chapter title; learning goals; brief motivation; chapter flow (`{tableofcontents}`).
- `1301-apis.ipynb`: REST API concepts and fundamentals; `requests.get()`; query parameters; error handling. Exercise: Weather API.
- `1302-api-reliability.ipynb`: POST requests; headers and API key patterns; nested JSON navigation; pagination; retries/backoff; response validation. Exercise: Resilient API Client.
- `MATERIALS.md`: teaching/assessment assets and prep checklist.

## Source of Truth

Use `chapters/13-apis/*` as the only Chapter 13 track for planning, delivery, and assessment.
